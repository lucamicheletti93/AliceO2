      ...
      for (const auto& binS : (*mBinsS.get())[layer1][layer2 - 1][cluster1.indexTableBin]) {
        ...
        for (std::vector<Cluster>::iterator it2 = (event.getClustersInLayer(layer2).begin() + clsMinIndexS); it2 != (event.getClustersInLayer(layer2).begin() + clsMaxIndexS + 1); ++it2) {
          ...
          for (Int_t point = 0; point < nPoints; ++point) {
            ...
            pippoClsSize = event.getPippoClusterSize(layer, cluster.clusterId);
            event.getCurrentTrack().setPoint(cluster, layer, clsInLayer, mcCompLabel, extClsIndex, pippoClsSize);
...
void Tracker<T>::addCellToCurrentTrackCA(const Int_t layer1, const Int_t cellId, ROframe<T>& event)
{
  ...
  if (trackCA.getNumberOfPoints() == 0) {
    ...
    pippoClsSize = event.getPippoClusterSize(layer2, cluster2.clusterId);
    trackCA.setPoint(cluster2, layer2, clsInLayer2, mcCompLabel2, extClsIndex, pippoClsSize);
  }

  extClsIndex = event.getClusterExternalIndex(layer1, cluster1.clusterId);
  pippoClsSize = event.getPippoClusterSize(layer2, cluster2.clusterId);
  trackCA.setPoint(cluster1, layer1, clsInLayer1, mcCompLabel1, extClsIndex, pippoClsSize);
}
